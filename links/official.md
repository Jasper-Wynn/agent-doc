# 官方文档链接

## Claude Code

| 资源 | 链接 |
|------|------|
| **官方首页** | https://code.claude.com |
| **文档首页** | https://code.claude.com/docs |
| **快速开始** | https://code.claude.com/docs/en/getting-started |
| **Context 管理** | https://code.claude.com/docs/en/context |
| **Skills** | https://code.claude.com/docs/en/skills |
| **Subagents** | https://code.claude.com/docs/en/subagents |
| **Agent Teams** | https://code.claude.com/docs/en/agent-teams |
| **并行工作** | https://code.claude.com/docs/en/parallel-work |
| **配置** | https://code.claude.com/docs/en/configuration |
| **MCP 集成** | https://code.claude.com/docs/en/mcp |

## Claude API

| 资源 | 链接 |
|------|------|
| **API 文档** | https://docs.anthropic.com |
| **SDK** | https://github.com/anthropics/anthropic-sdk-typescript |
| **Prompt 库** | https://docs.anthropic.com/prompt-library |
| **定价** | https://www.anthropic.com/pricing |

## MCP 协议

| 资源 | 链接 |
|------|------|
| **MCP 官网** | https://modelcontextprotocol.io |
| **规范文档** | https://spec.modelcontextprotocol.io |
| **服务器列表** | https://github.com/modelcontextprotocol/servers |
| **SDK** | https://github.com/modelcontextprotocol/sdk-typescript |

## 相关项目

| 项目 | 链接 |
|------|------|
| **Claude Code Skills** | https://github.com/anthropics/claude-code-skills |
| **MCP Servers** | https://github.com/modelcontextprotocol/servers |
